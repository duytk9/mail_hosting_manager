...
if [[ "$MODE" == "status" ]]; then
  ...
  step "Migrations"
  (cd "$APP_ROOT" && php scripts/migrate.php --status) || true
  exit 0
fi
...
as_root install -m 0640 -o root -g "$WEB_USER" "$SHARED_ENV" "$RELEASE_DIR/.env"
ok ".env in place"
...
if [[ "$SKIP_MIGRATE" == "0" ]]; then
  ...
  (cd "$RELEASE_DIR" && php scripts/migrate.php) \
