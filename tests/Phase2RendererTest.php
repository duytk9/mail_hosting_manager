...
final class Phase2RendererTest extends TestCase
{
    ...
    public function test_nginx_renderer_uses_external_webmail_root_alias(): void
    {
        ...
        $this->assertStringContainsString("script-src 'self';", $draft['content']);
        $this->assertStringContainsString("style-src 'self' https://fonts.googleapis.com;", $draft['content']);
        $this->assertStringNotContainsString("script-src 'self' 'unsafe-inline'", $draft['content']);
        $this->assertStringNotContainsString("style-src 'self' 'unsafe-inline'", $draft['content']);
