...
final class SuperAdminService
{
    ...
    public function syncAccess(int $userId, array $data): array
    {
        ...
        if ($resetPassword) {
            $generatedPassword = bin2hex(random_bytes(16)) . 'A1@';
            $this->passwordPolicy->assertStrong($generatedPassword);
        }
        ...
        if ($generatedPassword !== null) {
            $hash = $this->passwordHasher->hash($generatedPassword);
        ...
        $this->auditLog->log([
            ...
            'new_values' => [
                ...
                'ssh_enabled' => $sshEnabled,
                'ssh_sudo_enabled' => $sshSudoEnabled,
                'force_password_change' => $generatedPassword !== null ? 1 : (int) ($updated['force_password_change'] ?? 0),
            ],
        ]);
