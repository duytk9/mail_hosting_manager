...
{
    /**
     * Refuse to boot on a configuration where the admin/tenant hostname split is
     * enabled but silently ineffective.
     *
     * A SESSION_COOKIE_DOMAIN covering both hostnames (e.g. ".example.com") makes a
     * single session valid on both, so a tenant admin could simply visit the admin
     * hostname with their existing cookie. Failing loudly at boot is far better
     * than a split that looks configured but is not.
     */
    private static function assertHostSeparationIsCoherent(Config $config): void
    {
        $policy = new \MailPanel\Security\AdminHostPolicy(
            (string) $config->get('mailpanel.admin_hostname', ''),
            (string) $config->get('mailpanel.panel_hostname', ''),
        );
        ...
        $cookieDomain = (string) $config->get('app.session.cookie_domain', '');

        if ($policy->cookieDomainDefeatsSeparation($cookieDomain)) {
            throw new \RuntimeException(sprintf(
                'SESSION_COOKIE_DOMAIN "%s" covers both ADMIN_HOSTNAME (%s) and '
                . 'PANEL_HOSTNAME (%s), so one session would be valid on both hosts '
                . 'and the split would have no effect. Leave SESSION_COOKIE_DOMAIN empty.',
                $cookieDomain,
                $policy->adminHostname(),
    ...
    public static function create(string $basePath): Application
    {
        ...
        self::assertHostSeparationIsCoherent($config);
        ...
        $container->set(\MailPanel\Security\AdminHostPolicy::class, fn () => new \MailPanel\Security\AdminHostPolicy(
            (string) $config->get('mailpanel.admin_hostname', ''),
            (string) $config->get('mailpanel.panel_hostname', ''),
            (array) $config->get('mailpanel.admin_ip_allowlist', []),
            (bool) $config->get('mailpanel.admin_ip_allowlist_enforced', true)
        ));
        ...
        $container->set(NginxConfigRenderer::class, fn (Container $c) => new NginxConfigRenderer(
            ...
            (string) $config->get('mailpanel.admin_hostname', ''),
            (array) $config->get('mailpanel.admin_ip_allowlist', []),
            (int) $config->get('mailpanel.admin_https_port', 443)
        ));
