...
if [[ "$MODE" == "status" ]]; then
  step "Release state"
  remote "readlink -f '$APP_ROOT' 2>/dev/null || echo '(no current release)'"
  remote "ls -1t '$RELEASES_ROOT' 2>/dev/null | head -5 || echo '(no releases directory)'"
  step "Service state"
  remote "systemctl is-active nginx $PHP_FPM_SERVICE exim4 dovecot rspamd fail2ban 2>&1 | paste -d' ' - - - - - - || true"
  step "Pending migrations"
  remote "cd '$APP_ROOT' && php scripts/migrate.php --status 2>&1 || true"
  exit 0
fi
...
if [[ "$DRY_RUN" == "1" ]]; then
  ...
  remote "cd '$APP_ROOT' && php scripts/migrate.php --dry-run 2>&1 || true"
  ok "dry run complete; nothing was changed"
  exit 0
fi

step "Creating release directory on the server"
remote "install -d -m 0755 '$RELEASES_ROOT' '$RELEASE_DIR'
...
step "Linking the server .env into the release"
remote "set -euo pipefail
  if [[ -f '$APP_ROOT/.env' ]]; then
    cp -a '$APP_ROOT/.env' '$RELEASE_DIR/.env'
  elif [[ -f /etc/mailpanel/.env ]]; then
    cp -a /etc/mailpanel/.env '$RELEASE_DIR/.env'
  else
    echo 'No .env found on the server; create $APP_ROOT/.env before the first deploy.' >&2
    exit 1
  fi
  chmod 0640 '$RELEASE_DIR/.env'
  chown root:'$WEB_USER' '$RELEASE_DIR/.env'"
ok ".env in place"
...
if [[ "$SKIP_MIGRATE" == "0" ]]; then
  ...
  remote "cd '$RELEASE_DIR' && php scripts/migrate.php" || die "Migration failed. The release was NOT activated; the running site is untouched."
...
remote "cd '$RELEASES_ROOT' && ls -1t | tail -n +\$(( $KEEP_RELEASES + 1 )) | xargs -r rm -rf"
ok "done"
