...
#   sudo bash deploy/install.sh --reset-nginx      Back up and replace loaded Nginx config
#   sudo bash deploy/install.sh --preserve-nginx   Never reset existing Nginx config
#
...
NGINX_MANAGED_MARKER=/etc/mailpanel/nginx-managed
NGINX_BACKUP_ROOT=/root/mailpanel-nginx-backups
...
WITH_WEBMAIL=1
WITH_CLAMAV=1
NGINX_RESET_MODE=auto
NGINX_RESET_PENDING=0
NGINX_BACKUP_DIR=""
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --reset-nginx)
      [[ "$NGINX_RESET_MODE" != "preserve" ]] || { echo "--reset-nginx conflicts with --preserve-nginx" >&2; exit 2; }
      NGINX_RESET_MODE=force
      shift
      ;;
    --preserve-nginx)
      [[ "$NGINX_RESET_MODE" != "force" ]] || { echo "--preserve-nginx conflicts with --reset-nginx" >&2; exit 2; }
      NGINX_RESET_MODE=preserve
      shift
      ;;
...
mailpanel_manages_nginx() {
  [[ -f "$NGINX_MANAGED_MARKER" ]] && return 0
  [[ -f /etc/nginx/sites-available/mailpanel ]] \
    && grep -q '^# generated by MailPanel$' /etc/nginx/sites-available/mailpanel 2>/dev/null \
    && nginx -t >/dev/null 2>&1 \
    && systemctl is-active --quiet nginx
}

reset_nginx_configuration() {
  NGINX_BACKUP_DIR="$NGINX_BACKUP_ROOT/$(date -u +%Y%m%dT%H%M%SZ)-$$"
  install -d -m 0700 "$NGINX_BACKUP_DIR" || return 1
  tar -czf "$NGINX_BACKUP_DIR/etc-nginx.tar.gz" -C / etc/nginx || return 1
  chmod 0600 "$NGINX_BACKUP_DIR/etc-nginx.tar.gz" || return 1
  (
    cd "$NGINX_BACKUP_DIR"
    sha256sum etc-nginx.tar.gz >SHA256SUMS
...

  {
    ...
    printf 'reset_mode=%s\n' "$NGINX_RESET_MODE"
    printf 'loaded_sites:\n'
    find /etc/nginx/sites-enabled -mindepth 1 -maxdepth 1 -printf '  %f -> %l\n' 2>/dev/null | sort
    printf 'loaded_conf_d:\n'
    find /etc/nginx/conf.d -mindepth 1 -maxdepth 1 -name '*.conf' -printf '  %f -> %l\n' 2>/dev/null | sort
  } >"$NGINX_BACKUP_DIR/MANIFEST" || return 1
  chmod 0600 "$NGINX_BACKUP_DIR/MANIFEST" || return 1

  # From this point onward any error must restore the verified archive.
  NGINX_RESET_PENDING=1
  install -d -m 0700 "$NGINX_BACKUP_DIR/disabled/sites-enabled" "$NGINX_BACKUP_DIR/disabled/conf.d" || return 1
  while IFS= read -r -d '' entry; do
    mv -- "$entry" "$NGINX_BACKUP_DIR/disabled/sites-enabled/" || return 1
  done < <(find /etc/nginx/sites-enabled -mindepth 1 -maxdepth 1 -print0)
  while IFS= read -r -d '' entry; do
    mv -- "$entry" "$NGINX_BACKUP_DIR/disabled/conf.d/" || return 1
  done < <(find /etc/nginx/conf.d -mindepth 1 -maxdepth 1 -name '*.conf' -print0)

  nginx -t >>"$LOG_FILE" 2>&1
}

restore_nginx_configuration() {
  local archive="${NGINX_BACKUP_DIR}/etc-nginx.tar.gz"

  [[ -n "$NGINX_BACKUP_DIR" && "$NGINX_BACKUP_DIR" == "$NGINX_BACKUP_ROOT"/* ]] || return 1
  [[ -f "$archive" && -f "$NGINX_BACKUP_DIR/SHA256SUMS" ]] || return 1
  (cd "$NGINX_BACKUP_DIR" && sha256sum -c SHA256SUMS >/dev/null) || return 1

  install -d -m 0755 /etc/nginx/sites-available /etc/nginx/sites-enabled /etc/nginx/conf.d || return 1
  find /etc/nginx/sites-enabled -mindepth 1 -maxdepth 1 -exec rm -rf -- {} + || return 1
  find /etc/nginx/conf.d -mindepth 1 -maxdepth 1 -name '*.conf' -exec rm -rf -- {} + || return 1
  rm -f /etc/nginx/sites-available/mailpanel || return 1
...
rollback_pending_nginx_reset() {
  [[ "$NGINX_RESET_PENDING" == "1" ]] || return 0

  warn "installation stopped before the new Nginx configuration was confirmed; restoring the backup"
  if restore_nginx_configuration; then
    NGINX_RESET_PENDING=0
    ok "previous Nginx configuration restored"
    return 0
  fi

  warn "automatic Nginx restore failed; recover manually from $NGINX_BACKUP_DIR"
  return 1
}
...
on_install_exit() {
  ...
  if [[ "$status" != "0" ]]; then
    rollback_pending_nginx_reset || true
  fi
...
NGINX_RESET_REQUIRED=0
case "$NGINX_RESET_MODE" in
  force)
    NGINX_RESET_REQUIRED=1
    ;;
  preserve)
    NGINX_RESET_REQUIRED=0
    ;;
  auto)
    if ! mailpanel_manages_nginx; then
      NGINX_RESET_REQUIRED=1
    fi
    ;;
esac

if [[ "$NGINX_RESET_REQUIRED" == "1" ]]; then
  step "Backing up and resetting Nginx configuration"

  if reset_nginx_configuration; then
    ok "loaded Nginx sites disabled; backup: $NGINX_BACKUP_DIR"
  else
    if rollback_pending_nginx_reset; then
      die "Nginx base configuration is invalid. Previous configuration restored; see $LOG_FILE"
    fi

    die "Nginx base configuration is invalid and automatic restore failed. Recover from $NGINX_BACKUP_DIR"
  fi
else
  step "Preserving existing Nginx configuration"
  if mailpanel_manages_nginx; then
    skip "MailPanel already manages Nginx; no reset needed"
  else
    skip "Nginx reset disabled by --preserve-nginx"
  fi
fi

# MailPanel owns the default listeners. Refuse to continue when preservation
# leaves another enabled vhost claiming them; the later staging validation would
# fail with a less actionable "duplicate default server" error.
CONFLICTS="$(
  grep -rlE '^\s*listen[^;]*default_server' /etc/nginx/sites-enabled /etc/nginx/conf.d 2>/dev/null \
    | grep -vE '/mailpanel(\.conf)?$' \
    || true
)"
if [[ -n "$CONFLICTS" ]]; then
  printf '%s\n' "$CONFLICTS" | sed 's/^/       /' | tee -a "$LOG_FILE" >&2
  die "Existing Nginx vhosts still claim default_server. Re-run with --reset-nginx or resolve them first."
fi
...
if [[ -f "$SHARED_ENV" ]]; then
  ...
  # Self-heal missing TLS variables from older install scripts
  if ! grep -q '^EXIM_TLS_CERTIFICATE=' "$SHARED_ENV"; then
    echo "EXIM_TLS_CERTIFICATE=${TLS_SNI_ROOT}/${PANEL_HOSTNAME}.pem" >> "$SHARED_ENV"
    echo "EXIM_TLS_PRIVATEKEY=${TLS_SNI_ROOT}/${PANEL_HOSTNAME}.key" >> "$SHARED_ENV"
    ok "appended missing EXIM_TLS_* variables to $SHARED_ENV"
  fi
else
  cat >"$SHARED_ENV" <<ENV
...
EXIM_TLS_CERTIFICATE=${TLS_SNI_ROOT}/${PANEL_HOSTNAME}.pem
EXIM_TLS_PRIVATEKEY=${TLS_SNI_ROOT}/${PANEL_HOSTNAME}.key
...
step "Validating and activating Nginx"

NGINX_CHECK_OUT="$(mktemp /tmp/mailpanel-nginx-check-XXXXXX.log)"
NGINX_READY=0

if [[ ! -e /etc/nginx/sites-enabled/mailpanel && ! -L /etc/nginx/sites-enabled/mailpanel ]]; then
  printf 'MailPanel Nginx site was not activated.\n' >"$NGINX_CHECK_OUT"
elif ! nginx -t >"$NGINX_CHECK_OUT" 2>&1; then
  :
elif ! systemctl enable nginx >>"$NGINX_CHECK_OUT" 2>&1; then
  :
elif ! systemctl restart nginx >>"$NGINX_CHECK_OUT" 2>&1; then
  :
elif ! systemctl is-active --quiet nginx; then
  printf 'Nginx did not remain active after restart.\n' >>"$NGINX_CHECK_OUT"
else
  NGINX_READY=1
fi

tee -a "$LOG_FILE" <"$NGINX_CHECK_OUT"
rm -f "$NGINX_CHECK_OUT"

if [[ "$NGINX_READY" != "1" ]]; then
  if [[ "$NGINX_RESET_PENDING" == "1" ]]; then
    if rollback_pending_nginx_reset; then
      die "MailPanel Nginx activation failed. Previous configuration was restored; see $LOG_FILE"
    fi

    die "MailPanel Nginx activation and automatic restore both failed. Recover from $NGINX_BACKUP_DIR"
  fi

  die "MailPanel Nginx activation failed; existing configuration was left in place. See $LOG_FILE"
fi

install -d -m 0750 /etc/mailpanel
NGINX_MARKER_TMP="$(mktemp /tmp/mailpanel-nginx-managed-XXXXXX)"
{
  printf 'managed_at=%s\n' "$(date -u +%FT%TZ)"
  printf 'installer_root=%s\n' "$APP_ROOT"
  if [[ -n "$NGINX_BACKUP_DIR" ]]; then
    printf 'preinstall_backup=%s\n' "$NGINX_BACKUP_DIR"
  fi
} >"$NGINX_MARKER_TMP"
install -o root -g root -m 0644 "$NGINX_MARKER_TMP" "$NGINX_MANAGED_MARKER"
NGINX_RESET_PENDING=0
rm -f "$NGINX_MARKER_TMP"
ok "Nginx configuration is valid, active, and marked as MailPanel-managed"

# ------------------------------------------------------------------ services

step "Enabling and restarting services"

SERVICES=(php8.3-fpm mariadb exim4 dovecot rspamd redis-server fail2ban)
